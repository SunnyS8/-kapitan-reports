...
def generate_debt(filepaths: list[Path]) -> dict:
    ...
    # Вычисление просрочки если есть дата но нет дней
    if "date" in df.columns and "overdue_days" not in df.columns:
        df["overdue_days"] = df["date"].apply(
            lambda x: (now - x).days if pd.notna(x) else 0
        )
    ...
    summary = {
        ...
        "overdue_count": sum(1 for d in data if d["overdue_days"] > 90),
    }
    ...
    chart = {
        ...
        "values": [round(v, 2) for v in categories.values()],
    }
